...
<?php
    ...
    $Voltage2_12 = 0.00;//$data['Voltage 2 - 12 bit'];
    $Pulse = $data['Pulse'];

    echo $TimeStamp .' '. $RawData .' '. $Rssi .' '. $Snr .' '. $SequenceNumber .' '. $DeviceID .' '. $DeviceName .' '. $BatteryStatus .' '. $UseCase .' '. $BatteryVoltage .' '. $ApplicationId .' '.$Voltage1_12 .' '. $Voltage2_12 .' '. $Pulse; 
   
    
    //foreach($array as $row) //Extract the Array Values by using Foreach Loop
    //{
        $query .= "INSERT INTO evvos1(Time_Stamp, RawData, Rssi, Snr, SequenceNumber, DeviceID, DeviceName, BatteryStatus, UseCase, BatteryVoltage, ApplicationID, Voltage1, Voltage2, Pulse) VALUES ('$TimeStamp', '$RawData', '$Rssi', '$Snr', '$SequenceNumber', '$DeviceID', '$DeviceName', '$BatteryStatus', '$UseCase', '$BatteryVoltage', '$ApplicationId', '$Voltage1_12', '$Voltage2_12', '$Pulse'); ";
        echo $query;
